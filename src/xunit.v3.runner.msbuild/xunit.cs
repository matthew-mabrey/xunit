...
public class xunit : MSBuildTask, ICancelableTask, IDisposable
{
	...
	/// <summary/>
	public string? ParallelizationOptions { get; set; }
	...
	async Task<bool> ExecuteAsync()
	{
		...
		if (!TryParseOptionalEnum<ParallelizationOptions>(ParallelizationOptions, "ParallelizationOptions value '{0}' is invalid", out var parallelizationOptions))
			return false;
		...
		using (AssemblyHelper.SubscribeResolveForAssembly(typeof(xunit), globalDiagnosticsMessageSink))
		{
			...
			try
			{
				...
				foreach (var assembly in Assemblies)
				{
					...
					if (parallelizationOptions.HasValue)
				...
				var parallelizeAssemblies = project.Assemblies.All(assembly =>
					assembly.Configuration.ParallelizationOptions?.HasFlag(Sdk.ParallelizationOptions
						.Assemblies) == true);
