...
public static class ParallelismOptionsExtensions
{
	...
	/// <summary>
	/// Determines the parallelism options to use for a test collection.
	/// </summary>
	/// <param name="assemblyOptions">The parallelism options for the assembly of the test collection.</param>
	/// <param name="collectionOptions">The parallelism options for the test collection.</param>
	/// <returns>The determined parallelism options for the test collection.</returns>
	/// <remarks>
	/// Priority: <paramref name="assemblyOptions"/> when <see cref="ParallelismOptions.None"/> > <paramref name="collectionOptions"/> > <paramref name="assemblyOptions"/>
	/// </remarks>
	public static ParallelismOptions DetermineParallelismOptions(
		this ParallelismOptions assemblyOptions,
		ParallelismOptions? collectionOptions)
	{
		if (assemblyOptions == ParallelismOptions.None)
		{
			return assemblyOptions;
		}

		return collectionOptions ?? assemblyOptions;
	}
}
